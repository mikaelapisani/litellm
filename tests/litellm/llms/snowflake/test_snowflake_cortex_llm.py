...
def test_completion_success(mock_post, mock_snowflake_llm, tmp_path, mock_oauth_token):
    ...
    mock_response.iter_lines.return_value =[b"data: {\"choices\": [{\"delta\": {\"content\": \"Hello\"}}], \"usage\": {\"prompt_tokens\":15, \"completion_tokens\": 40}}\n"]
...
@patch("requests.post")
def test_completion_success_several_lines(mock_post, mock_snowflake_llm, tmp_path, mock_oauth_token):
    """Test a successful completion request."""
    mock_response = MagicMock()
    mock_response.headers = {"Content-Type": "text/event-stream"} 
    mock_response.iter_lines.return_value =[b"data: {\"choices\": [{\"delta\": {\"content\": \"Hello.\"}}], \"usage\": {\"prompt_tokens\":15, \"completion_tokens\": 40}}\n",
    ...
    print('response:', response.choices)
    assert response.model == "mistral-large2"
    assert response.choices[0]["message"].content == "Hello. how can I help you? I am here."
...
def test_format_callback(mock_post, mock_snowflake_llm, tmp_path, mock_oauth_token):
    ...
    mock_format_callback = MagicMock(return_value=[{"role": "user", "content": "Modified message"}])
    ...
    assert request_payload["messages"] == [{"role": "user", "content": "Modified message"}]  # Validate transformation


@patch("requests.post")
def test_generate_id_callback(mock_post, mock_snowflake_llm, tmp_path, mock_oauth_token):
    """Test a successful completion request."""
    mock_response = MagicMock()
    mock_response.headers = {"Content-Type": "text/event-stream"} 
    mock_response.iter_lines.return_value =[b"data: {\"choices\": [{\"delta\": {\"content\": \"Hello\"}}], \"usage\": {\"prompt_tokens\":15, \"completion_tokens\": 40}}]\n"]
